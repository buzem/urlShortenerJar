...
/**
 * Test cases to verify the {@link URLShortenerClient} of the rest-binding
 * module. It performs these steps in order. 1. Runs an embedded Tomcat
 * server with a mock RESTFul web service. 2. Invokes the {@link URLShortenerClient}
...
public class RestClientTest {
  ...
  private static URLShortenerClient rc = new URLShortenerClient();
  private static final String RESPONSE_TAG = "url";
  ...
  public static void init() throws IOException, DBException, ServletException, LifecycleException, InterruptedException {
    String webappDirLocation =  RestClientTest.class.getClassLoader().getResource("WebContent").getPath();
    ...
    props.load(new FileReader(RestClientTest.class.getClassLoader().getResource("workload_url_shortener").getPath()));
